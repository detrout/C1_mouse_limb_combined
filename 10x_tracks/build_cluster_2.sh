...
cat <(samtools view -H $RUN1/outs/possorted_genome_bam.bam) \
    <(grep -f barcodes-10x-cluster-2-run-1.txt <(samtools view $RUN1/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-3.txt <(samtools view $RUN3/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-4.txt <(samtools view $RUN4/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-5.txt <(samtools view $RUN5/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-6.txt <(samtools view $RUN6/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-7.txt <(samtools view $RUN7/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-8.txt <(samtools view $RUN8/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-12.txt <(samtools view $RUN12/outs/possorted_genome_bam.bam)) \
    <(grep -f barcodes-10x-cluster-2-run-13.txt <(samtools view $RUN13/outs/possorted_genome_bam.bam)) | \
      samtools view -@ 4 -bS - -o 10x-cluster-2_unsorted.bam
